...
const int keyerPin = 9;     // the number of the keyer pin
const int ledPin =  19;      // the number of the LED pin

bool keyState = false;         // variable for reading the keyer status
...
//|||||||||||||||||||||||||||||||||||||||||||
// keyline service and characteristic
NimBLERemoteService* pSvc = nullptr;
NimBLERemoteCharacteristic* pChr = nullptr;
NimBLEClient* pClient = nullptr;
//|||||||||||||||||||||||||||||||||||||||||||
static bool exitProg = false;
static bool doConnect = false;
static uint32_t scanTime = 0; /** 0 = scan forever */


/**  None of these are required as they will be handled by the library
     with defaults. **
 **                       Remove as you see fit for your needs   */
class ClientCallbacks : public NimBLEClientCallbacks {
    void onConnect(NimBLEClient* pClient) {
      Serial.println("Connected");
      /** After connection we should change the parameters if we
          don't need fast
          response times.
          These settings are 150ms interval, 0 latency, 450ms timout.
          Timeout should be a multiple of the interval, minimum is 100ms.
          I find a multiple of 3-5 * the interval works best
          for quick response/reconnect.
          Min interval: 120 * 1.25ms = 150, Max interval: 120 * 1.25ms = 150,
          0 latency, 60 * 10ms = 600ms timeout
      */
      ...
      */
      pClient->updateConnParams(1, 1, 0, 60);
    };
    ...
    */
                                   ...
                                   const ble_gap_upd_params* params)
    {
      ...
      { /** 1.25ms units */
        ...
        return false;
      } else if (params->latency > 2) { /** Num of intervals allowed to skip */
        return false;
    ...
    ****** Note: these are the same return values as defaults ********/
    ...
    /** Pairing process complete, we can check the results in
        ble_gap_conn_desc */
              ...
              uint8_t* pData, size_t length, bool isNotify)
{
  ...
  str += std::string(pRemoteCharacteristic->
                     getRemoteService()->getClient()->getPeerAddress());
  str += ": Service = " +
...
bool connectToServer()
{
  ...
  if (!pClient) {
    ...
    /** Set initial connection parameters: These settings are 15ms interval,
        0 latency, 120ms timout.
        These settings are safe for 3 clients to connect reliably,
        can go faster if you have less
        connections. Timeout should be a multiple of the interval,
        minimum is 100ms.
        Min interval: 12 * 1.25ms = 15, Max interval: 12 * 1.25ms = 15,
        0 latency, 51 * 10ms = 510ms timeout
    */
  ...
  if (pSvc) {    /** make sure it's not null */
    ...
    pChr = pSvc->getCharacteristic(characteristicUUID);
    ...
    if (pChr) {    /** make sure it's not null */
      ...
      */
      if (pChr->canNotify()) {
        ...
        /** Send false as first argument to subscribe to indications instead
          of notifications */
  ...
  {
    Serial.println("KEY service not found.");
  }
...
void setup () {
  ...
  /** Set the IO capabilities of the device, each option will
      trigger a different
      pairing method.
...
void loop ()
{
  ...
  if (connectToServer())
  {
    Serial.println("Success! now be getting notifications, scanning for more");
    ...
    {
      //delay(1000);
      //keyerPin = false, keyed
      //keyerPin = true, unkeyed
      //keyState = true, radio keyed
      //keyState = false, radio unkeyed

      if ((digitalRead(keyerPin) == 1))//pulled high
      {
        //UNKEYED!!!!
        //
        if (keyState == false)
        {
          //unkeyed and keystate is false, do nothing
        }
        else
        { //unkeyed and keystate is true, we need to unkey

          if (pChr->writeValue('0'))//tell the server to unkey
          {
            //Serial.print("Wrote zero to: ");
            //Serial.println(pChr->getUUID().toString().c_str());
            keyState = false;
            digitalWrite(ledPin, LOW);
          }
      ...
      }//end keyer pin unkeyed

      if ((digitalRead(keyerPin) == 0))//asserted low
      {
        //KEYED!!!!
        //
        if (keyState == true)
        {
          //keyed and keystate is true, do nothing
        }
        else
        { //keyed and keystate is false, we need to key

          if (pChr->writeValue('1'))//tell the server to key
          {
            //Serial.print("Wrote zero to: ");
            //Serial.println(pChr->getUUID().toString().c_str());
            keyState = true;
